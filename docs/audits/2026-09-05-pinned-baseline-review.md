...
|---:|---|---|---|
| 1 | `adversarial-review-fanout` | 기준 mock은 `survives`만 반환한다. 새 투표는 명시적 status와 관측 evidence/reason이 있어야 유효하다. 따라서 옛 mock으로는 confirmed finding이 나오지 않는다. | mock을 새 응답 계약으로 이전. 기존 severity 순서·cap·누락 보고 assertion 유지, 정족수·missing lane·전체 예산 회귀 추가 |
| 1 | `dotenv-allowlist` | 기준 fixture는 이전 hook helper 일부만 복사한다. 새 hook의 dependency closure가 빠져 dotenv 읽기까지 도달하지 않는다. | 실제 helper closure를 격리 fixture에 복사. hostile dotenv가 실제 읽혔는지와 NODE_OPTIONS가 실행되지 않았는지 모두 확인. allowlist 복사본 일치도 별도 검사 |
...
| 3 | `loop-fix-fail-channel`, `loop-fix-infra-exempt`, `loop-fix-mark-clean` | 기준 verifier가 자기 카운터만 바꾸고 no-op fixer 뒤 PASS를 내는 시나리오는 실제 구현 수정의 증거가 아니다. | 안정된 verifier와 Git에 보이는 실제 wrong→fixed 변경으로 fixture 교체. infra 제외·실패 신호·중복 방지·검증된 교훈과 후속 clean 관측 assertion 유지 |
...
확인했다. 검토된 소스의 순환 신뢰 위험을 숨기지 않기 위해 이 FAIL 결과를 최종 보고서에도
